...
const NOT_LOGGED_IN = (res: Response) => res.status(401).json({ status: "error", result: "User not logged in" });

const NOT_FOUND = (res: Response) => res.status(404).json({ status: "error", result: "Not found" });

const PERMISSIONS_FAIL = (r: Response): Response =>
  r.status(401).json({
    status: "error",
